...
#
#  This file converts src/js/*.js to a C-array in src/nodemcujs_js.[h|c] file.
# And this file also generates magic string list in src/nodemcujs_string_ext.inl.h
...
def js2c(buildtype, js_modules, snapshot_tool=None, verbose=False):
    ...
    str_const_regex = re.compile('^#define NODEMCUJS_MAGIC_STRING_\w+\s+"(\w+)"$')
    with open(fs.join(path.SRC_ROOT, 'nodemcujs_magic_strings.h'), 'r') as fin_h:
        for line in fin_h:
    ...
    with open(fs.join(path.SRC_ROOT, 'nodemcujs_js.h'), 'w') as fout_h, \
         open(fs.join(path.SRC_ROOT, 'nodemcujs_js.c'), 'w') as fout_c:
    ...
    magic_str_path = fs.join(path.SRC_ROOT, 'nodemcujs_string_ext.inl.h')
